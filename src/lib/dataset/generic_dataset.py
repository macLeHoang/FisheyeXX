...
from utils.image import gaussian_radius, draw_umich_gaussian, draw_umich_gaussian_1d
from utils.image import get_intersection
from utils.FisheyeLoader import FisheyeLoader 
from utils.FisheyeTools import xywha2vertex
...
class GenericDataset(data.Dataset):
  ...
  def __init__(self, opt=None, split=None, ann_path=None, img_dir=None):
    ...
    if ann_path is not None and img_dir is not None:
      ...
      if opt.fisheye and not opt.fisheye_pretrain:
        self.fLoader = FisheyeLoader(ann_path, sample_rate=opt.fsampling, split=split)
      else: 
        self.coco = coco.COCO(ann_path)

      if opt.fisheye and opt.fisheye_pretrain:
        self.images = self.coco.getImgIds(catIds=[1])
      elif opt.fisheye:
        self.images = self.fLoader.get_imgIds()
      else:
      ...
      if opt.tracking:
        ...
        if opt.fisheye and not opt.fisheye_pretrain:
          _dataset = self.fLoader.anns
        else:
          _dataset = self.coco.dataset
        for image in _dataset['images']:
  ...
  def __getitem__(self, index):
    ...
    ret = {'image': inp, 'img_info': img_info}
    gt_det = {'bboxes': [], 'scores': [], 'clses': [], 'cts': []}
    cost_volume_track_ids = []
    cost_volume_pre_cts_wo_disturb = []

    pre_cts, track_ids = None, None
    pre_cts_wo_disturb = None
    if opt.tracking:
      if self.split == 'train':
        pre_image_list, pre_anns_list, frame_dist_list = self._load_pre_data_train(
                                                                img_info['video_id'], 
                                                                img_info['frame_id'],
                                                                img_info['sensor_id'] if 'sensor_id' in img_info else 1, 
                                                                num_frames=self.opt.clip_len)
        for prev_id, (pre_image, pre_anns, frame_dist) in enumerate(zip(pre_image_list, pre_anns_list, frame_dist_list)):
          if flipped:
          ...
          pre_hm, pre_cts_tmp, track_ids_tmp, pre_cts_wo_disturb_tmp = self._get_pre_dets(
            pre_anns, trans_input_pre, trans_output_pre)
          # pre_cts = pre_cts_tmp
          # track_ids = track_ids_tmp
          cost_volume_track_ids.append(copy.deepcopy(track_ids_tmp))
          cost_volume_pre_cts_wo_disturb.append(copy.deepcopy(pre_cts_wo_disturb_tmp))
          if prev_id == 0:
            pre_cts = copy.deepcopy(pre_cts_tmp)
            track_ids = copy.deepcopy(track_ids_tmp)
            pre_cts_wo_disturb = copy.deepcopy(pre_cts_wo_disturb_tmp)
          ret['pre_img_{}'.format(prev_id+1)] = pre_img
          # cv2.imwrite('trainimg/{}_pre_img_{}.jpg'.format(index, prev_id+1), debug_im)
          if opt.pre_hm:
            ret['pre_hm_{}'.format(prev_id+1)] = pre_hm
      else:
        ...
        if opt.same_aug_pre and frame_dist != 0:
          ...
          trans_output_pre = get_affine_transform(
            c_pre, s_pre, rot, [opt.output_w, opt.output_h])
        pre_img = self._get_input(pre_image, trans_input_pre)
        pre_hm, pre_cts, track_ids, pre_cts_wo_disturb = self._get_pre_dets(
          pre_anns, trans_input_pre, trans_output_pre)
        ret['pre_img'] = pre_img
        if opt.pre_hm:
          ret['pre_hm'] = pre_hm
    ...
    self._init_ret(ret, gt_det, num_objs) # init all samples here
    ...
    for k in range(num_objs):
      ...
      if self.opt.fisheye:
        bbox = self._get_bbox_output_fisheye(ann['bbox'], 
                                             trans_output, 
                                             height, width,
                                             rot)
        bbox_amodal = None
      ...
      if self.opt.seg:
        if self.opt.fisheye and not self.opt.fisheye_pretrain:
          instance_mask = self.fLoader.ann_to_mask(ann)
        else:
      ...
      if self.opt.fisheye:
        pass # TODO not support fisheye yet
      else:
  ...
  def _load_fisheye_image_anns(self, img_id, fLoader, img_dir):
    img_info = fLoader.load_imgs(imgIds=[img_id])[0]
    root = img_info['root']
    folder = img_info['folder']
    name = img_info['file_name']
    img_path = os.path.join(img_dir, root, folder, name)

    anns = copy.deepcopy(fLoader.load_anns(imgIds=[img_id]))
    img = cv2.imread(img_path)
    return img, anns, img_info, img_path
  ...
  def _load_data(self, index):
    if self.opt.fisheye and not self.opt.fisheye_pretrain:
      ...
      img, anns, img_info, img_path = self._load_fisheye_image_anns(img_id, fLoader, img_dir)
    else:
      coco = self.coco
      img_dir = self.img_dir
      img_id = self.images[index]
  ...
  def _load_pre_data_train(self, video_id, frame_id, sensor_id=1, num_frames=3):
    ...
    for i in range(num_frames-1):
      if img_ids[rand_id][1] < frame_id:
        rand_id2 = np.random.choice(len(img_ids))

        if abs(img_ids[0][1] - frame_id) <= (num_frames - 1) or (not self.opt.no_repeat):
          while img_ids[rand_id2][1] > frame_id:
            rand_id2 = np.random.choice(len(img_ids))
        else:
          while img_ids[rand_id2][1] > frame_id or rand_id2 in rand_ids:
            rand_id2 = np.random.choice(len(img_ids))
      else:
        rand_id2 = np.random.choice(len(img_ids))

        if abs(img_ids[-1][1] - frame_id) <= (num_frames - 1) or (not self.opt.no_repeat):
          while img_ids[rand_id2][1] < frame_id:
            rand_id2 = np.random.choice(len(img_ids))
        else:
          while img_ids[rand_id2][1] < frame_id or rand_id2 in rand_ids:
            rand_id2 = np.random.choice(len(img_ids))
      rand_ids[i+1] = rand_id2

    if num_frames > 1:
      if (rand_ids <= self.opt.max_frame_dist - 1).all():
    ...
    frame_dist_list = []
    for i in rand_ids:
      img_id, pre_frame_id = img_ids[i]
      frame_dist = abs(frame_id - pre_frame_id)
      if self.opt.fisheye:
        img, anns, _, _ = self._load_fisheye_image_anns(img_id, self.fLoader, self.img_dir)
      else:
        img, anns, _, _ = self._load_image_anns(img_id, self.coco, self.img_dir)
      img_list.append(img)
      anns_list.append(anns)
      frame_dist_list.append(frame_dist)

    return img_list, anns_list, frame_dist_list
  ...
  def _get_pre_dets(self, anns, trans_input, trans_output):
    ...
    for ann in anns:
      ...
      if cls_id > self.opt.num_classes or cls_id <= -99 or \
        ...
        continue
      if self.opt.fisheye and not self.opt.fisheye_pretrain:
        oriented_angle, bbox = self._coco_box_to_bbox_fisheye(ann['bbox'])
        points = np.array([[bbox[0], bbox[1]], [bbox[2], bbox[3]],
                           [bbox[4], bbox[5]], [bbox[6], bbox[7]]], dtype=np.float32)
        for t in range(4):
          points[t] =  affine_transform(points[t], trans)
        points[:, 0] = np.clip(points[:, 0], 0, hm_w - 1)
        points[:, 1] = np.clip(points[:, 1], 0, hm_h - 1)

        xmin_amodel, xmax_amodel = points[:, 0].min(), points[:, 0].max()
        ymin_amodel, ymax_amodel = points[:, 1].min(), points[:, 1].max()

        x_arg_sorted = np.argsort(points[:, 0])
        xmin, y_xmin = points[x_arg_sorted[0]]
        x_ymin_tmp, ymin_tmp = points[x_arg_sorted[1]]
        x_ymax_tmp, ymax_tmp = points[x_arg_sorted[2]]
        xmax, y_xmax = points[x_arg_sorted[3]]

        # Assign coordinates to 4 points with coordinates coressponding to \
        # points with xmin, xmax, ymin, ymax 
        if y_xmin == ymax_amodel:
          xmin, y_xmin = points[x_arg_sorted[1]]
          x_ymin_tmp, ymin_tmp = points[x_arg_sorted[0]]
        if y_xmax == y_xmin:
          x_ymax_tmp, ymax_tmp = points[x_arg_sorted[3]]
          xmax, y_xmax = points[x_arg_sorted[2]]
        if ymin_tmp > ymax_tmp:
          x_ymin, ymin, x_ymax, ymax = x_ymax_tmp, ymax_tmp, x_ymin_tmp, ymin_tmp
        else:
          x_ymin, ymin, x_ymax, ymax = x_ymin_tmp, ymin_tmp, x_ymax_tmp, ymax_tmp
        
        points[0] = xmin, y_xmin
        points[1] = x_ymin, ymin
        points[2] = xmax, y_xmax
        points[3] = x_ymax, ymax

        w1 = ((points[1] - points[0])**2).sum() **0.5
        w2 = ((points[2] - points[3])**2).sum() **0.5
        w = min(w1, w2)

        h1 = ((points[3] - points[0])**2).sum() **0.5
        h2 = ((points[2] - points[1])**2).sum() **0.5
        h = min(h1, h2)
      ...
      if (h > 0 and w > 0):
        ...
        if self.opt.fisheye:
          ct = (points[0] + points[1] + points[2] + points[3]) / 4
        else:
          ct = np.array(
        ...
        pre_cts_wo_disturb.append(ct0 / down_ratio)
    ...
    return pre_hm, pre_cts, track_ids, pre_cts_wo_disturb
  ...
  def _get_aug_param(self, c, s, width, height, disturb=False):
    ...
    if self.opt.fisheye:
      if np.random.random() < self.opt.fisheye_rot:
  ...
  def _flip_anns(self, anns, width):
    for k in range(len(anns)):
      ...
      if self.opt.fisheye and not self.opt.fisheye_pretrain:
        anns[k]['bbox'] = [
          width - bbox[0] - 1, bbox[1], bbox[2], bbox[3], -bbox[4]]
      else:
        anns[k]['bbox'] = [
  ...
  def _init_ret(self, ret, gt_det, num_objs):
    ...
    ret['cat_down'] = np.zeros((max_objs), dtype=np.int64)
    ret['mask_down'] = np.zeros((max_objs), dtype=np.float32)
    ret['h_hm_down'] = np.ones((int((self.opt.output_h / 2) * (self.opt.output_w / 2)), int(self.opt.output_h / 2)), dtype=np.float32)
    ret['w_hm_down'] = np.ones((int((self.opt.output_h / 2) * (self.opt.output_w / 2)), int(self.opt.output_w / 2)), dtype=np.float32)
    ret['h_ind_down'] = np.zeros((max_objs), dtype=np.int64)
    ret['w_ind_down'] = np.zeros((max_objs), dtype=np.int64)
    for temporal_id in range(2, self.opt.clip_len):
      ret['h_hm_down_prev{}'.format(temporal_id)] = np.ones((int((self.opt.output_h / 2) * (self.opt.output_w / 2)), int(self.opt.output_h / 2)), dtype=np.float32)
      ret['w_hm_down_prev{}'.format(temporal_id)] = np.ones((int((self.opt.output_h / 2) * (self.opt.output_w / 2)), int(self.opt.output_w / 2)), dtype=np.float32)
      ret['h_ind_down_prev{}'.format(temporal_id)] = np.zeros((max_objs), dtype=np.int64)
      ret['w_ind_down_prev{}'.format(temporal_id)] = np.zeros((max_objs), dtype=np.int64)
      ret['cat_down_prev{}'.format(temporal_id)] = np.zeros((max_objs), dtype=np.int64)
      ret['mask_down_prev{}'.format(temporal_id)] = np.zeros((max_objs), dtype=np.float32)


    regression_head_dims = {
      'angle': 1, 'reg': 2, 'wh': 2, 'tracking': 2, 'ltrb': 4, 'ltrb_amodal': 4, 
      'nuscenes_att': 8, 'velocity': 3, 'hps': self.num_joints * 2, 
      'dep': 1, 'dim': 3, 'amodel_offset': 2}

    for head in regression_head_dims:
      if head in self.opt.heads: # (hm, wh, regr, angle)
        ret[head] = np.zeros(
  ...
  def _coco_box_to_bbox_fisheye(self, box):
    if self.opt.fisheye_pretrain:
      bbox = self._coco_box_to_bbox(box)
      oriented_angle = 0
    else:
      box[-1] = box[-1] 
      oriented_angle = box[-1]
      bbox = xywha2vertex(np.array([box]), False, stack=False)
      bbox = bbox[0]

    return oriented_angle, bbox
  ...
  def _get_bbox_output_fisheye(self, bbox, trans_output, height, width, rot):
    oriented_angle, bbox = self._coco_box_to_bbox_fisheye(bbox)
    bbox = bbox.copy()

    if not self.opt.fisheye_pretrain:
      points = np.array([[bbox[0], bbox[1]], [bbox[2], bbox[3]],
                        [bbox[4], bbox[5]], [bbox[6], bbox[7]]], dtype=np.float32)
    else:
      points = np.array([[bbox[0], bbox[1]], [bbox[0], bbox[3]],
                       [bbox[2], bbox[3]], [bbox[2], bbox[1]]], dtype=np.float32)

    for t in range(4):
      points[t] =  affine_transform(points[t], trans_output)
    ...
    flag_out_of_box = np.any(points < 0) or \
                      np.any(points[:, 0] > self.opt.output_w - 1) or \
                      np.any(points[:, 1] > self.opt.output_h - 1)
    # if object's parts lie outside the frame
    if flag_out_of_box:
      w, h = self.opt.output_w, self.opt.output_h
      if xmin >= w-1 or xmax <= 0 or ymin >= h-1 or ymax <= 0:
        return np.array([0,0,0,0,0])
      else:
        point_list = get_intersection(points, w, h, oriented_angle, rot, self.opt.fisheye_pretrain)
        return np.array(point_list)

    # if the object lies fully inside the frame
    xc, yc = (points[0] + points[1] + points[2] + points[3]) / 4 # x_center, y_center
    # oriented angle
    oriented_angle = oriented_angle-rot
    ...
    return np.array([xc-w/2, yc-h/2, xc+w/2, yc+h/2, oriented_angle])


  def _add_instance(self, 
                    ret, gt_det, k, cls_id, 
                    bbox, bbox_amodal, ann, 
                    trans_output,
                    aug_s, calib, 
                    pre_cts=None, 
                    track_ids=None, 
                    pre_cts_wo_disturb=None,
                    cost_volume_track_ids=None, 
                    cost_volume_pre_cts_wo_disturb=None, 
                    instance_mask=None):
    h, w = bbox[3] - bbox[1], bbox[2] - bbox[0]
    if h <= 0 or w <= 0:
      return
    ...
    if self.opt.trades:
      if ann['track_id'] in track_ids:
        pre_ct_wo_disturb = pre_cts_wo_disturb[track_ids.index(ann['track_id'])]
        self.get_cost_volume_label(bbox, k, ret, pre_ct_wo_disturb)
      assert len(cost_volume_track_ids) == self.opt.clip_len
      for temporal_id in range(2, self.opt.clip_len):
        if ann['track_id'] in cost_volume_track_ids[temporal_id-1]:
          pre_ct_wo_disturb = cost_volume_pre_cts_wo_disturb[temporal_id-1][cost_volume_track_ids[temporal_id-1].index(ann['track_id'])]
          self.get_cost_volume_label_aux(bbox, k, ret, pre_ct_wo_disturb, temporal_id)
    ...
    if 'angle' in ret:
      ret['angle'][k] = bbox[-1] / 180.0 * np.pi
      ret['angle_mask'][k] = 1
  ...
  def get_cost_volume_label(self, bbox, k, ret, pre_ct):
    pre_ct = pre_ct / 2
    ...
    # 1D cost_volume
    cls_id = int(ct_int[1] * (self.opt.output_w / 2) + ct_int[0])
    ret['h_hm_down'][cls_id] = 0
    ret['w_hm_down'][cls_id] = 0
    ret['cat_down'][k] = cls_id
    ret['mask_down'][k] = 1
    ret['h_ind_down'][k] = pre_ct_int[1]
    ret['w_ind_down'][k] = pre_ct_int[0]

    hm_tmp = np.zeros((int(self.opt.output_h / 2), int(self.opt.output_w / 2)),dtype=np.float32)
    draw_umich_gaussian_1d(hm_tmp, pre_ct_int, radius)
  ...
  def get_cost_volume_label_aux(self, bbox, k, ret, pre_ct, temporal_id):
    pre_ct = pre_ct / 2
    pre_ct_int = pre_ct.astype(np.int32)
    bbox = bbox / 2
    h, w = bbox[3] - bbox[1], bbox[2] - bbox[0]
    ...
    # 1D cost_volume
    cls_id = int(ct_int[1] * (self.opt.output_w / 2) + ct_int[0])
    ret['h_hm_down_prev{}'.format(temporal_id)][cls_id] = 0
    ret['w_hm_down_prev{}'.format(temporal_id)][cls_id] = 0
    ret['cat_down_prev{}'.format(temporal_id)][k] = cls_id
    ret['mask_down_prev{}'.format(temporal_id)][k] = 1
    ret['h_ind_down_prev{}'.format(temporal_id)][k] = pre_ct_int[1]
    ret['w_ind_down_prev{}'.format(temporal_id)][k] = pre_ct_int[0]

    hm_tmp = np.zeros((int(self.opt.output_h / 2), int(self.opt.output_w / 2)),dtype=np.float32)
    draw_umich_gaussian_1d(hm_tmp, pre_ct_int, radius)
    ret['h_hm_down_prev{}'.format(temporal_id)][cls_id][:] = hm_tmp[:, pre_ct_int[0]]
    ret['w_hm_down_prev{}'.format(temporal_id)][cls_id][:] = hm_tmp[pre_ct_int[1], :]
    ret['h_hm_down_prev{}'.format(temporal_id)][cls_id][pre_ct_int[1]] = 0
    ret['w_hm_down_prev{}'.format(temporal_id)][cls_id][pre_ct_int[0]] = 0
