...
from model.losses import PeriodicSmoothL1Loss
from model.losses import PeriodicAngleLoss
from model.losses import BinRotLoss, WeightedBCELoss
from model.decode import generic_decode
from model.utils import _tanh, _sigmoid, flip_tensor, flip_lr_off, flip_lr
...
class GenericLoss(torch.nn.Module):
  def __init__(self, opt):
    ...
    self.crit_angle = PeriodicAngleLoss()
  ...
  def _tanh_output(self, output):
    if 'angle' in output:
      output['angle'] = _tanh(output['angle']) * np.pi
      return output
  ...
  def forward(self, outputs, batch):
    ...
    for s in range(opt.num_stacks):
      ...
      output = self._tanh_output(output)
      ...
      if 'angle' in output:
        losses['angle'] += self.crit_angle(
          output['angle'], batch['angle'], 
          batch['mask'], batch['ind']) / opt.num_stacks
...
class Trainer(object):
  ...
  def _get_losses(self, opt):
    loss_order = ['hm', 'wh', 'reg', 'angle', 'ltrb', 'hps', 'hm_hp', \
