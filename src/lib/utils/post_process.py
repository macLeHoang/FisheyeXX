...
from .FisheyeTools import xywha2vertex, iou_rle
      ...
      if 'bboxes' in dets:
        if opt.fisheye:
          bbox = transform_preds_with_trans(
            dets['bboxes'][i][j][:-1].reshape(2, 2), trans).reshape(4)
          angle = dets['bboxes'][i][j][-1]
          bbox = np.append(bbox, angle)
          item['bbox'] = bbox

          if opt.box_nms > 0:
            det_boxes[j, :] = bbox 
            if len(keep) > 0:
              overlap = iou_rle(bbox[None, :], det_boxes[keep, :], 
                                img_size = (height, width), 
                                xyxy=True, is_degree=False)
