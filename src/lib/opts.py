...
class opts(object):
  def __init__(self):
    ...
    self.parser.add_argument('--angle_weight', type=float, default=0.1,
                             help='loss weight for angle oriented \
                                   if working with fisheye images.')
    ...
    # fisheye
    self.parser.add_argument('--fisheye', action='store_true', default=False,
                              help='working with fisheye images.')
    self.parser.add_argument('--fisheye_rot', type=float, default=0.99,
                              help='probability of applying rotation.')
    self.parser.add_argument('--fisheye_pretrain', action='store_true', default=False,
                              help='Pretrain with COCO dataset')
    self.parser.add_argument('--fisheye_saveroot', type=str, default='',
                              help='Save root.')
    # images taken from high FPS camera -> resampling to reduce redundants
    self.parser.add_argument('--fsampling', type=int, default=-1,
                              help='resamples fisheye dataset') 

    # fTesting
    self.parser.add_argument('--rand_rot_test', action='store_true', default=False,
                              help='testing with random rotation')    
    self.parser.add_argument('--rand_scale_test', action='store_true', default=False,
                              help='testing with random scaling')                                                
  ...
  def update_dataset_info_and_set_heads(self, opt, dataset):
    ...
    weight_dict = {'hm': opt.hm_weight, 'wh': opt.wh_weight,
                   ...
                   'seg_feat':1.0}
    if opt.fisheye:
        weight_dict.update({'angle': opt.angle_weight})
  ...
  def init(self, args=''):
    ...
    if opt.fisheye:
      default_dataset_info['ctdet_fisheye'] = 'coco'
      # default_dataset_info['fisheye'] = 'coco'
