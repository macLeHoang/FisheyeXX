...
def generic_decode(output, K=100, opt=None):
  ...
  cur_det = {'cts': cts, 'inds':inds, 'scores': scores}
  ...
  if 'wh' in output:
    ...
    if wh.size(2) == 2 * cat: # cat spec
      ...
      #   idx = wh[..., 0] > wh[..., 1]
      #   wh[idx][..., 0], wh[idx][..., 1] = wh[idx][..., 1], wh[idx][..., 0]
    else:
      pass
  ...
  if 'angle' in output: 
    ...
    angle = angle.view(batch, K, 1)
    if 'wh' in output:
      bboxes = torch.cat([ret['bboxes'], angle], dim=2)
      ret['bboxes'] = bboxes
  ...
  regression_heads = ['tracking', 'dep', 'rot', 'dim', 'amodel_offset',
    'nuscenes_att', 'velocity', 'embedding']

  for head in regression_heads:
    if head in output:
      if 'embedding' in head:
        ret[head] = _tranpose_and_gather_feat(
          F.normalize(output[head], dim=1), inds).view(batch, K, -1)
      else:
        ret[head] = _tranpose_and_gather_feat(
