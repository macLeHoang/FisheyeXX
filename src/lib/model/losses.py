...
# from piou_loss.pixel_weights import Pious
...
class PeriodicSmoothL1Loss(nn.Module):
  ...
  def forward(self, out, target, mask, ind):
    ''' Period Smooth L1 Loss
    Arguments:
      out: Angle out branch of model (H/down_ratio, W/down_ratio, 1)
      target: batch x max_objects x dim
      mask: batch x max_objects
      ind: batch x max_objects
    '''
    pred = _tranpose_and_gather_feat(out, ind)
    theta = pred - target

    mask = mask.unsqueeze(2).expand_as(target)
    theta = theta[mask==1]
    arctan = torch.atan2(torch.sin(theta), torch.cos(theta))

    zeros = torch.zeros_like(arctan, requires_grad=False)
    loss = F.smooth_l1_loss(arctan, zeros, reduction='sum')
    loss = loss / (mask.float().sum() + 1e-4)
    return loss

class PeriodicAngleLoss(nn.Module):
  def __init__(self):
    super(PeriodicAngleLoss, self).__init__()
  
  def forward(self, out, target, mask, ind):
    '''Period Angle Loss - From RAPiD
    Arguments:
      out: Angle out branch of model (H/down_ratio, W/down_ratio, 1)
      target: batch x max_objects x dim
      mask: batch x max_objects
    ...
    '''
    pred = _tranpose_and_gather_feat(out, ind)
    theta = pred - target - np.pi/2

    mask = mask.unsqueeze(2).expand_as(target)
    theta = theta[mask==1]

    remainder = torch.remainder(theta, np.pi)
    loss = torch.abs(remainder - np.pi/2)
    loss = loss.sum() / (mask.float().sum() + 1e-4)
    return loss
...
#     self.template = template_w_pixels(512)
